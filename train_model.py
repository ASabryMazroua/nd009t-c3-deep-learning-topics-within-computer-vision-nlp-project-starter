...
def test(model, test_loader, criterion, hook):
    ...
    '''
    model.eval()
    hook.set_mode(smd.modes.EVAL) #Set the SMDebug hook for the test phase
    test_loss = 0
    correct = 0
    with torch.no_grad():
...
def main(args):
    ...
    '''
    model=net()
    hook = smd.Hook.create_from_json_file() #Creating the hook we'll use in tracking
    hook.register_hook(model) #Registering the model to the hook
    
    '''
    TODO: Create your loss and optimizer
    '''
    criterion = nn.CrossEntropyLoss() #We'll use the Cross Entropy as a Loss Function
    ...
    model = train(model, train_loader, validation_loader, criterion, optimizer, hook) #Training the model
    ...
    path = os.path.join(args.model_dir, "final_model.pth")
    torch.save(model, path)

if __name__=='__main__':
    parser=argparse.ArgumentParser(description="Dog Breed Classification with PyTorch")
